...
public class CustomerAPIJUnitIntegrationTest {
    ...
    public void ensureThatUserAPICallReturnStatusCode200() throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://localhost:8090/api/customers")).build();
    ...
    void ensureThatJsonIsReturnedAsContentType() throws Exception {
        HttpClient client = HttpClient.newBuilder().build();
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://localhost:8090/api/customers/1")).build();
        HttpResponse<String> response = client.send(request, BodyHandlers.ofString());
